...
Author:Elham 10-Jul-2025  start modifications
"""
...
# ─────────────────────────── settings ──────────────────────────────────────
# TEMPLATE_PATH = pathlib.Path("hackaton2025/182x0046_SE_24.0_B.POL")
TEMPLATE_PATH = pathlib.Path("hackaton2025/Prueba n46 original.pol")
...
import re
_TOKEN_RE = re.compile(r"\b(first|second|third|beamNo)\b")
# whole-word match of the   first   second   third  tokens
_PLACE_RE = re.compile(r'\b(first|second|third)\b')

import re

# matches the placeholder words even when embedded in other text
_PLACE_RE = re.compile(r'(first|second|third|beamNo)')
...
def _swap_placeholders(lines: list[str], v: dict[str, float]) -> None:
    ...
    with the appropriate numeric text, without altering anything else.
        ...
        second  → |CA2_1p|
        third   → |CA1_1p|
        beamNo  → number of DISTINCT non-zero half-arm lengths
    """
    ...
    repl = {
        "first":  f"{abs(v['CA3_1p']):g}",
        "second": f"{abs(v['CA2_1p']):g}",
        "third":  f"{abs(v['CA1_1p']):g}",
        "beamNo": str(len(uniq)),
    }
...
_CA_PLACEHOLDER_RE = re.compile(r"'CROSSARM-(first|second|third)'")
...
def patch_pol(template: pathlib.Path, dest: pathlib.Path,
    ...
    try:
        zz0 = next(i for i, ln in enumerate(lines) if ZERO_ZERO_RE.match(ln))
    ...
    # replace first / second / third / beamNo everywhere
